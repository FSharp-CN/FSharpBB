...
import { ofFunc, tuple } from "./prelude";
...
export function parseParams(querystring) {
  return create(choose(function (x) {
    ...
    return map_3(function (segment) {
      return toKeyValuePair(segment);
    }, source);
  }(split(querystring.substr(1), ["&"], null, 0))), new Comparer(comparePrimitives));
}
